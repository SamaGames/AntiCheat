...
public class Calculations {
    ...
    public static void Obfuscate(PacketChunkData packet, Player player) {
        ...
        ComputeChunkInfoAndObfuscate(info);
        packet.getChunkMap().a = info.original;
        packet.getChunkMap().b = info.chunkMask;
    }

    public static ChunkInfo[] getInfo(PacketChunkDataBulk packet, Player player) {
        ChunkData[] chunks = new ChunkData[packet.getChunksMap().length];
        ChunkInfo[] infos = new ChunkInfo[chunks.length];

        for (int i = 0; i < chunks.length; i++) {
            // Create an info objects
            chunks[i] = new ChunkData(packet.getChunksMap()[i], packet.getLocsX()[i], packet.getLocsY()[i]);
    ...
    public static ChunkInfo getInfo(PacketChunkData packet, Player player)
    {
        return new ChunkInfo(player, new ChunkData(packet.getChunkMap(), packet.getLocX(), packet.getLocY()), buffer.get());
    }
